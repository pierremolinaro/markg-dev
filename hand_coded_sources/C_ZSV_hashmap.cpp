...
C_ZSV_hashmap::cAllocInfo C_ZSV_hashmap::smAllocInfo ;
...
void * C_ZSV_hashmap::MyBlockavltree_element_for_collision::operator new (const size_t /*inByteSize*/) {
...
void C_ZSV_hashmap::MyBlockavltree_element_for_collision::operator delete (void * inPtr) {
...
void C_ZSV_hashmap::C_TreeForCollision::allocBlock (void) {
//--- Realloc block list ?
...
C_ZSV_hashmap::C_TreeForCollision::C_TreeForCollision (void) :
mRoot (NULL) {
}
...
size_t C_ZSV_hashmap::getAllocatedSizeInBytes (void) {
...
void C_ZSV_hashmap::C_TreeForCollision::
...
C_ZSV::cZSVinfo * C_ZSV_hashmap::C_TreeForCollision::
avltree_search (MyBlockavltree_element_for_collision * ioRootPointer,
                 const C_ZSV::cZSVinfo & inInfo) {
  C_ZSV::cZSVinfo * result = NULL ;
  if (ioRootPointer == NULL) {
    result = (C_ZSV::cZSVinfo *) NULL ;
  }else{
    const intptr_t comp = ioRootPointer->mInfo.compare (inInfo) ;
    if (comp > 0) {
...
C_ZSV::cZSVinfo * C_ZSV_hashmap::C_TreeForCollision::
search_or_insert (const C_ZSV::cZSVinfo & inInfo,
                  bool & outInsertionPerformed) {
  bool unused = false ;
...
C_ZSV::cZSVinfo * C_ZSV_hashmap::C_TreeForCollision::
search (const C_ZSV::cZSVinfo & inInfo) {
...
size_t C_ZSV_hashmap::C_TreeForCollision::internalRecursiveSweep (C_ZSV_hashmap::MyBlockavltree_element_for_collision * inElement) {
  size_t sweepedNodes = 0 ;
  if (inElement != NULL) {
...
size_t C_ZSV_hashmap::C_TreeForCollision::sweepUnmarkedObjects (void) {
...
recursiveTransfertElementsInNewMapArray (MyBlockavltree_element_for_collision * const inElementPointer,
  ...
  if (inElementPointer != NULL) {
    ...
    inElementPointer->mBalance = 0 ;
    const uintptr_t hash = inElementPointer->mInfo.getHashCodeForMap () % inNewSize ;
    bool extension ; // Unused
    bool insertionPerformed ; // Unused
...
size_t C_ZSV_hashmap::C_TreeForCollision::
...
C_ZSV_hashmap::C_ZSV_hashmap (void) :
...
C_ZSV::cZSVinfo * C_ZSV_hashmap::
search_or_insert (const C_ZSV::cZSVinfo & inInfo,
                  bool & outInsertionPerformed) {
  uintptr_t hashCode = inInfo.getHashCodeForMap () ;
...
size_t C_ZSV_hashmap::sweepUnmarkedObjects (void) {
  size_t sweepedNodes = 0 ;
...
size_t C_ZSV_hashmap::getMapSizeInBytes (void) const {
  return mEntryCurrentCount * sizeof (C_TreeForCollision) ;
}

//---------------------------------------------------------------------------*

void C_ZSV_hashmap
::reallocMap (const size_t inNewSize) {
  const size_t newSize = (inNewSize < 1) ? 1 : getPrimeGreaterThan ((uint32_t) inNewSize) ;
...
void C_ZSV_hashmap::unmarkAllObjects (void) {
  for (size_t i=0 ; i<mEntryCurrentCount ; i++) {
...
size_t C_ZSV_hashmap::getMarkedNodesCount (void) const {
  size_t count = 0 ;
  for (size_t i=0 ; i<mEntryCurrentCount ; i++) {
